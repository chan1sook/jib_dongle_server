...
import {
  getPaths,
  validatorDockerComposePath,
  getLocalLighthousePath,
  getLighhouseDownloadUrl,
  validatorDockerComposeGroup,
  getChainConfigPath,
  isOverrideCheckFiles,
  getChainConfigGitUrl,
  getLighhouseSha256Checksum,
  getChainConfigGitSha256Checksum,
  getChainConfigDir,
} from "../constant";
...
export default function deployVcsAction(socket: import("socket.io").Socket) {
  return async (
    keyFileContent: Record<string, string>,
...
async function deployValidators(
  ...
  try {
    ...
    const isChainConfigFileValid =
      !isOverrideCheckFiles() &&
      (await isFileValid(chainConfigPath, getChainConfigGitSha256Checksum()));
    ...
    const envContent =
      ...
      `NODE_GRAFFITI="${
        advanceSetting.graffiti.replace(/\"/g, "_") || "JBCValidatorClient"
      }"\n` +
    ...
    // Get jibchain data (and script)
    deployVcLogger.logInfo(
      "LIGHTHOUSE_EXEC_PATH",
      filePaths.LIGHTHOUSE_EXEC_PATH
    );

    const lhFilePath = getLocalLighthousePath();
    const isLhFileValid =
      !isOverrideCheckFiles() &&
      (await isFileValid(lhFilePath, getLighhouseSha256Checksum()));
    ...
    if (!isLhFileValid) {
      ...
      deployVcLogger.injectExecTerminalLogs(
        await basicExec(
          "curl",
          ["-L", getLighhouseDownloadUrl(), "-o", "lighthouse.tar.gz"],
          {
      ...
      deployVcLogger.injectExecTerminalLogs(
        await basicExec("tar", ["-xvf", "lighthouse.tar.gz"], {
          cwd: filePaths.LIGHTHOUSE_EXEC_PATH,
        })
      );
    ...
    const importKeyPromise = new Promise<DeployKeyResult>((resolve, reject) => {
      const importKeyProcess = spawnProcess(
        lhFilePath,
        [
          "account",
          "validator",
      ...
      importKeyProcess.on("exit", async (code, signal) => {
        if (code === 0) {
          ...
          const captureOutText =
            /Successfully imported ([0-9]+) validators? \(([0-9]+) skipped\)\./.exec(
              out
            );
          if (captureOutText) {
          ...
          const err = new Error(
            tokens[tokens.length - 1] || `Exit code: ${code || signal}`
          );
          reject(err);
        }
      });
    ...
    const vcDefsFilePath = path.join(
      vcKeyExportPath,
      "/validators/validator_definitions.yml"
    );
    const vcDefsContentBuffer = await fs.readFile(vcDefsFilePath);
    const vcDefsContentStr = vcDefsContentBuffer.toString();

    const newVcDefsContentStr = stringReplaceAll(
      vcDefsContentStr,
      vcKeyExportPath,
      "/root/.lighthouse/custom"
    );
    ...
    await sudoExec(
      `docker compose -p ${dockerComposeProjectGroup} -f ${validatorDockerComposePath()} down
      docker container rm -f jbc-validator
      cp -rf ${path.join(filePaths.VC_DEPLOY_TEMP, "config")} ${
        filePaths.VC_KEYS_PATH
      }
    ...
    const apiKeyPath = path.join(
      vcKeyMountPath,
      "custom/validators/api-token.txt"
    );
    ...
    for (let retry = 0; retry < maxRetry; retry += 1) {
      try {
        apiOut = await basicExec("cat", [apiKeyPath]);
